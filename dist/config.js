...
function required(name) {
    ...
    if (!value) {
        throw new Error(`Hiányzó környezeti változó: ${name}. Töltsd ki a .env fájlban (lásd .env.example).`);
    }
    return value;
}
export const config = {
    navEnv: (process.env.NAV_ENV ?? "test"),
    baseUrl: (process.env.NAV_ENV ?? "test") === "production"
