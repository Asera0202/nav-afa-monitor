...
import { computePasswordHash, computeRequestSignature, currentTimestamp, decryptExchangeToken, generateRequestId, } from "./nav-auth.js";
const xmlParser = new XMLParser({ ignoreAttributes: false, attributeNamePrefix: "@_" });
/** Közös fejléc (header + user + software) XML-blokk minden nem-manageInvoice kéréshez. */
function buildCommonHeaderXml() {
...
async function postXml(endpoint, bodyXml) {
...
export async function tokenExchange() {
    ...
    const encryptedToken = root?.encodedExchangeToken;
    if (!encryptedToken) {
...
export async function queryInvoiceDigest(query) {
    const { xml } = buildCommonHeaderXml();
 ...
 */
export async function queryInvoiceData(invoiceNumber, direction = "INBOUND", batchIndex) {
    const { xml } = buildCommonHeaderXml();
